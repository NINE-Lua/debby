...
local rblx = version and version()
...
check("clipboardAccess", (function()
    local success, result = pcall(function()
        if typeof(setclipboard) ~= "function" or typeof(getclipboard) ~= "function" then
            return false
        end
        local testString = "DebbyClipboardTest_" .. tostring(math.random(1000,9999))
        setclipboard(testString)
        task.wait(0.1)
        return getclipboard() == testString
    end)
    return success and result
end)())
local finish = tick()
...
print("\n \n Excess Info; \n [?] Roblox Version:", rblx, "\n [?] Executor Name:", name, "\n [?] Time Taken: " .. string.format("%.2f", finish - start) .. "s")
if percent >= 90 then
...
if veright == true then
    ...
    warn("[!] You were using the wrong version of debby, the executor test results may be outdated.")
end
